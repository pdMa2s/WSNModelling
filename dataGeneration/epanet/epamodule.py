...
if _plat=='Linux':
  try:
      # _lib = ctypes.CDLL("../epanet/mylibepanet2.so")
  ...
  try:
    ...
    _lib = ctypes.CDLL("epanet\epanet2.dll")
...
def ENepanet(nomeinp, nomerpt='', nomebin='', vfunc=None):
    """Runs a complete EPANET dataGeneration.
...
def ENsetlinkvalue(index, paramcode, value):
    ...
    paramcode: Link parameter codes consist of the following constants:
                 ...
                 * Use EN_INITSTATUS and EN_INITSETTING to set the design value for a link's status or setting that 
                   exists prior to the start of a dataGeneration. Use EN_STATUS and EN_SETTING to change these values while
                   a dataGeneration is being run (within the ENrunH - ENnextH loop).
...
def ENsolveH():
    """Runs a complete hydraulic dataGeneration with results
...
def ENinitH(flag=None):
    ...
    and the dataGeneration clock time prior
...
def ENrunH():
    ...
    retrieving the current dataGeneration clock time t"""
    ierr= _lib.ENrunH(ctypes.byref(_current_simulation_time))
    if ierr >= 100: raise ENtoolkitError(ierr)
    # elif 0 < ierr < 100: print(ENtoolkitError(ierr))
    return _current_simulation_time.value


def ENsimtime():
    """retrieves the current dataGeneration time t as datetime.timedelta instance"""
...
def ENsolveQ():
    """Runs a complete water quality dataGeneration with results
...
def ENinitQ(flag=None):
    """Initializes water quality and the dataGeneration clock
...
def ENnextQ():
    """Advances the water quality dataGeneration
...
def ENstepQ():
    """Advances the water quality dataGeneration one water quality time step.
    The time remaining in the overall dataGeneration is returned in tleft."""
    tleft= ctypes.c_long()
...
def ENsaveH():
    """Transfers results of a hydraulic dataGeneration
...
def ENreport():
    """Writes a formatted text report on dataGeneration results
    to the Report file."""
    ierr= _lib.ENreport()
    if ierr!=0: raise ENtoolkitError(ierr)
