...
def demand_flows(start_time, end_time):

    return Caudal_R(start_time, end_time), Caudal_VC(start_time, end_time)
...
def benchmark2018(x, iChart, step=3600, hF0=4, duration=24):
    nInc = len(x)
    # definição dos dicionários
    empty_timeIncrem = {
        'number': None,
        'startTime': None, 'duration': None, 'endTime': None,
        'hFini': [], 'hFfin': [], 'E': -1, 'dmds': [], 'pumps': -1}
    ...
    for i in range(0, nInc):
        ...
        timeInc[i]['duration'] = duration / nInc
        ...
        QR, QVC = demand_flows(timeInc[i]['startTime'], timeInc[i]['endTime'])
    ...
    if iChart == 1:
        ...
        y1.insert(nInc, timeInc[nInc-1]['hFfin'])
...
if __name__ == '__main__':
    ...
    fObjRest, sensibil, time_inc = benchmark2018(x, 1, duration=48, step=3600)
